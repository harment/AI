...
// ---- Update question ----
if ($action === 'update' && isset($_POST['qid'])) {
    $qid     = (int)$_POST['qid'];
    ...
    if ($qtext && $optA && $optB && $optC && $optD) {
        $db->prepare("UPDATE questions SET question_text=?,option_a=?,option_b=?,option_c=?,option_d=?,correct_option=?,feedback_correct=?,feedback_wrong=? WHERE id=? AND lesson_id=?")
           ->execute([$qtext, $optA, $optB, $optC, $optD, $correct, $fbOk, $fbWrong, $qid, $lessonId]);
        $msg = 'تم تحديث السؤال.';
    }
}

// ---- Delete question ----
if ($action === 'delete' && isset($_POST['qid'])) {
    $db->prepare("DELETE FROM questions WHERE id=? AND lesson_id=?")->execute([(int)$_POST['qid'], $lessonId]);
...
if ($action === 'delete_all') {
    $db->prepare("DELETE FROM questions WHERE lesson_id=?")->execute([$lessonId]);
    $msg = 'تم حذف جميع الأسئلة.'; $msgType = 'danger';
}
...
<head>
  ...
  <title>تعديل أسئلة الدرس – <?= clean($lesson['name']) ?></title>
...
<main class="main-content">
  <div style="margin-bottom:.75rem;font-size:.88rem;color:var(--muted);">
    ...
    <strong>تعديل الأسئلة</strong>
  </div>
  <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:1.5rem;flex-wrap:wrap;gap:1rem;">
    <h2><i class="fas fa-edit"></i> تعديل أسئلة: <?= clean($lesson['name']) ?></h2>
    <div style="display:flex;gap:.75rem;flex-wrap:wrap;">
      ...
      <a href="/teacher/questions.php?lesson_id=<?= $lessonId ?>" class="btn btn-outline btn-sm"><i class="fas fa-eye"></i> عرض الأسئلة</a>
  ...
  <div style="margin-bottom:1rem;display:flex;align-items:center;gap:.75rem;flex-wrap:wrap;">
    ...
    <form method="POST" style="display:inline;" onsubmit="return confirm('سيتم حذف جميع الأسئلة! هل أنت متأكد؟')">
      <input type="hidden" name="action" value="delete_all">
      <button type="submit" class="btn btn-danger btn-sm"><i class="fas fa-trash-alt"></i> حذف الكل</button>
    </form>
    <?php endif; ?>
  </div>
  ...
  <div style="display:flex;flex-direction:column;gap:1.25rem;">
    ...
    <div class="card" style="padding:1.25rem;">
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:1rem;">
        <span style="background:var(--dark);color:#fff;border-radius:50%;width:28px;height:28px;display:inline-flex;align-items:center;justify-content:center;font-size:.85rem;font-weight:700;"><?= $i+1 ?></span>
        <div style="display:flex;gap:.5rem;">
          <button class="btn btn-primary btn-sm" onclick="openEditModal(<?= htmlspecialchars(json_encode($q), ENT_QUOTES) ?>)">
            <i class="fas fa-edit"></i> تعديل
          </button>
          <form method="POST" style="display:inline;" onsubmit="return confirm('حذف هذا السؤال؟')">
      ...
      <div style="font-weight:700;margin-bottom:.75rem;font-size:1rem;"><?= clean($q['question_text']) ?></div>
  ...
  <div style="margin-top:1.5rem;display:flex;gap:.75rem;flex-wrap:wrap;">
    ...
    <a href="/teacher/questions.php?lesson_id=<?= $lessonId ?>" class="btn btn-outline">
      <i class="fas fa-arrow-right"></i> عرض الأسئلة
    </a>
  </div>
</main>
...
<div id="editModal" style="display:none;" class="modal-backdrop">
  <div class="modal-box">
    <div class="modal-header">
      <span class="modal-title">تعديل السؤال</span>
    ...
    <form method="POST" id="editForm">
      ...
      <div class="form-group"><label class="form-label">نص السؤال *</label><textarea name="question_text" id="editQtext" class="form-control" rows="2" required></textarea></div>
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:.75rem;">
        <div class="form-group"><label class="form-label">أ) الخيار الأول *</label><input type="text" name="option_a" id="editOptA" class="form-control" required></div>
        <div class="form-group"><label class="form-label">ب) الخيار الثاني *</label><input type="text" name="option_b" id="editOptB" class="form-control" required></div>
        <div class="form-group"><label class="form-label">ج) الخيار الثالث *</label><input type="text" name="option_c" id="editOptC" class="form-control" required></div>
        <div class="form-group"><label class="form-label">د) الخيار الرابع *</label><input type="text" name="option_d" id="editOptD" class="form-control" required></div>
      </div>
      <div class="form-group">
        <label class="form-label">الإجابة الصحيحة *</label>
        <select name="correct_option" id="editCorrect" class="form-control">
          <option value="a">أ</option><option value="b">ب</option><option value="c">ج</option><option value="d">د</option>
        </select>
      </div>
      <div class="form-group"><label class="form-label">تغذية راجعة عند الإجابة الصحيحة</label><input type="text" name="feedback_correct" id="editFbOk" class="form-control" placeholder="رسالة تشجيعية…"></div>
      <div class="form-group"><label class="form-label">تغذية راجعة عند الإجابة الخاطئة</label><input type="text" name="feedback_wrong" id="editFbWrong" class="form-control" placeholder="شرح الإجابة الصحيحة…"></div>
...
function openEditModal(q) {
  ...
  document.getElementById('editFbOk').value     = q.feedback_correct || '';
  document.getElementById('editFbWrong').value  = q.feedback_wrong || '';
